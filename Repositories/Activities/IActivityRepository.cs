...
namespace Repositories.Activities
{
	...
	Task<List<(int UserId, string FullName, string Email, bool? IsPresent, int? ParticipationScore, string? CheckInMethod, DateTime? GpsCheckInTime, DateTime? GpsCheckOutTime, double? CheckInDistance)>> GetRegistrantsWithAttendanceAsync(int activityId);
	...
	Task<List<ActivityAttendance>> GetAttendancesWithScoreAsync(int activityId);
