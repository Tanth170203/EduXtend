...
public class ClubMovementRecordService : IClubMovementRecordService
{
        ...
        bool isCollaboration)
    {
        try
        {
            ...
            // Update the appropriate score category
            var previousTotal = record.TotalScore;
            UpdateScoreCategory(record, scoreCategory, actualPoints);

            // Recalculate TotalScore and cap at 100
            record.TotalScore = record.ClubMeetingScore + record.EventScore + record.CompetitionScore +
                               record.PlanScore + record.CollaborationScore;

            if (record.TotalScore > 100)
            {
                _logger.LogInformation("[AWARD TO CLUB] Total score {TotalScore} exceeds 100 for Club {ClubId}. Capping at 100.",
                    record.TotalScore, clubId);
            ...
            try
            {
                ...
                _logger.LogInformation("[AWARD TO CLUB] Updated ClubMovementRecord {RecordId}. Previous total: {PreviousTotal}, New total: {NewTotal}",
                    record.Id, previousTotal, record.TotalScore);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[AWARD TO CLUB] Error updating ClubMovementRecord {RecordId} for Club {ClubId}. Error: {ErrorMessage}",
                    record.Id, clubId, ex.Message);
                throw;
            }

            // Create ClubMovementRecordDetail
            try
            {
                ...
                _logger.LogInformation("[AWARD TO CLUB] Created ClubMovementRecordDetail for Club {ClubId}, Activity {ActivityId}, Score={Score}, Category={Category}",
